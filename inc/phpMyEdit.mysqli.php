...
/**
 * line 1486, re: class btn-primary see also http://v4-alpha.getbootstrap.com/components/buttons/
 * function htmlSubmit() modified to use 'btn btn-primary btn-sm' Bootstrap style in addition to the default phpMyEdit style.
 * Bootstrap class form-control added to INPUT, TEXTAREA, SELECT (SELECT also includes  style="width:auto;")
 * Bootstrap TABLE class applied as class="table table-responsive table-sm " followed by the phpMyEdit class name
 * Use your text editor to search for "Bootstrap tweak" (without the quotes).
 * SQL connection function commented out below (this utilizes MySQLi improved connection in /inc/pme.config.php).
 * An external abort() function is utilized.
 * ereg_replace is deprecated, changed below to preg_replace
 * $escape was missing from one function: function htmlTextarea()
 *    $escape = isset($this->fdd[$k]['escape']) ? $this->fdd[$k]['escape'] : true;
 * Varoius HTML hacks applied, e.g.
 *    Commented out below: <hr> tags
 *    Changed below <h1> & <h4> converted to <h3 class="text-danger"> Twitter Bootstrap style.
 *    added id= parameter to various HTML tags
 *    added title="" parameter to certain links, alt="" parameter to graphic links
 *    rel="nofollow" applied to links in table header, to prevent Google from sorting records thousands of times
 */

/*
 * phpMyEdit - instant MySQL table editor and code generator
 *
 * phpMyEdit.mysqli.php - main table editor class definition file
 ...
 * Copyright (c) 2002-2006 Platon Group, http://opensource.platon.sk/
 * All rights reserved.
 *
 ...
 * http://opensource.platon.sk/projects/phpMyEdit/
 */

/* A modified version of: $Platon: phpMyEdit/phpMyEdit.class.php,v 1.208 2007-09-22 14:45:50 michal Exp $ */
...
class phpMyEdit
{
	...
	function col_has_URL($k)    { return isset($this->fdd[$k]['URL']) || isset($this->fdd[$k]['URLprefix']) || isset($this->fdd[$k]['URLpostfix']); }
	function col_has_multiple($k) { return $this->col_has_multiple_select($k) || $this->col_has_checkboxes($k); }
	function col_has_multiple_select($k) { return $this->fdd[$k]['select'] == 'M' && ! $this->fdd[$k]['values']['table']; }
	function col_has_checkboxes($k) { return $this->fdd[$k]['select'] == 'C' && ! $this->fdd[$k]['values']['table']; }
	function col_has_radio_buttons($k) { return $this->fdd[$k]['select'] == 'O' && ! $this->fdd[$k]['values']['table']; }
	function col_has_datemask($k) { return isset($this->fdd[$k]['datemask']) || isset($this->fdd[$k]['strftimemask']); }
	...
	{
		return $val === null || (isset($this->fdd[$k]['values2']) && !isset($this->fdd[$k]['values']['table']));
	}
	...
	function debug_var($name, $val) 
	{
		if (is_array($val) || is_object($val)) {
			...
			echo '<p>debug_var()::<i>',htmlspecialchars($name),'</i>';
			echo '::<b>',htmlspecialchars($val),'</b>::',"</p>\n";
		}
	}

	/*
	 * Certain sql functions disabled, connection made prior to inclusion of this file
     */

	// unused ... see pme.config.php
	// function sql_connect() { $this->dbh = @ini_get('allow_persistent') ? @mysqli_pconnect($this->hn, $this->un, $this->pw) : @mysqli_connect($this->hn, $this->un, $this->pw); }
	// function sql_disconnect() { if ($this->close_dbh) { @mysqli_close($this->dbh); $this->dbh = null; } }
	...
	function myquery($qry, $line = 0, $debug = 0)
	{
		...
		if ($debug_query || $debug) {
			$line = intval($line);
			echo '<h3 class="text-danger">MySQL query at line ',$line,'</h3>',htmlspecialchars($qry),"\n";
		}

		if(empty($this->dbh)){
			echo '<h3 class="text-danger">Database handle cannot be found: $opts[\'dbh\']</h3>',"\n";
		}else{
			$ret = mysqli_query($this->dbh, $qry);
			if (! $ret) {
				$errors[] = mysqli_error($this->dbh);
				if($qry){ $errors[] = $qry; }
				abort($errors);
			}
		}
	...
	function make_language_labels($language) 
	{
		...
		$small = array( 'Search' => 'v', 'Hide'   => '^', 'Clear'  => 'X', 'Query'  => htmlspecialchars('>'));
		if ((!$this->nav_text_links() && !$this->nav_graphic_links()) || !isset($ret['Search']) || !isset($ret['Query']) || !isset($ret['Hide'])   || !isset($ret['Clear'])) {
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	return true;
		...
		if ($this->display['form']) {
			...
			echo ' action="',$page_name,'" name="'.$this->cgi['prefix']['sys'].'form" id="'.$this->cgi['prefix']['sys'].'form">',"\n";
		}
		return true;
	}
	...
	function display_tab_labels($position) 
	{
		...
		// display tabs; Bootstrap tweaks applied in TABLE, TR, A
		echo '<table class="table-sm table-bordered table-inverse">',"\n";
		echo '<tr class="table-info"><td class="table-info text-primary">Sections:</td>',"\n";
		for ($i = ($this->tabs[0] == 'PMEtab0' ? 1 : 0); $i < count($this->tabs); $i++) {
			...
			echo '<a class="text-primary ',$css_class_name,'" id="'.$this->dhtml['prefix'].'tab',$i,'_',$position,'_link';
	...
	function display_add_record() 
	{
		for ($tab = 0, $k = 0; $k < $this->num_fds; $k++) {
			if (isset($this->fdd[$k]['tab']) && $this->tabs_enabled() && $k > 0) {
				...
				// Bootstrap tweak: table
				echo '<table class="table table-responsive table-sm ',$this->getCSSclass('main'),'">',"\n";
			}
			if (! $this->displayed[$k]) {
				continue;
			}
			...
			if ($this->col_has_values($k)) {
				...
				echo ' id="',$this->cgi['prefix']['data'].$this->fds[$k],'"';
...
// Bootstrap tweak form-control
			echo '<input class="form-control ',$css_class_name,'" type="text"';
			echo ($this->readonly($k) ? ' disabled' : '');
	...
	function display_password_field($row, $k) 
	{
		...
		echo '<input class="form-control ',$this->getCSSclass('value', null, true, $css_postfix),'" type="password"';
		echo ($this->readonly($k) ? ' disabled' : '');
	...
	function cellDisplay($k, $row, $css) 
	{
		...
		if (intval($this->fdd[$k]['trimlen']) > 0 && strlen($value) > $this->fdd[$k]['trimlen']) {
			...
			// 2010-06-08 16:09 looks to work ok but uncertain if this is optimal
	...
	function htmlSubmit($name, $label, $css_class_name, $js_validation = true, $disabled = false, $js = NULL) 
	{
		...
		// Bootstrap tweak
		$ret = '<input'.$markdisabled.' type="submit" class="btn btn-primary btn-sm '.$css_class_name .'" name="'.$this->cgi['prefix']['sys'].ltrim($markdisabled).$name .'" value="'.(isset($this->labels[$label]) ? $this->labels[$label] : $label);
		if ($js_validation) {
	...
	{
		$ret = '<select class="form-control '.htmlspecialchars($css).'" style="width:auto;" name="'.htmlspecialchars($name);
		if ($multiple) {
	...
	{
		$ret = '<textarea class="form-control '.$css.'" id="'.$name.'" name="'.$name.'"';
	...
	{
		// hack for change log REMOTE_USER (author's system uses $_SESSION['sess_user_name'])
		//if($name == 'REMOTE_USER'){
	...
	function display_list_table_buttons($position, $listall = false) 
	{
		...
		// Bootstrap tweak: table
		echo '<table class="table table-responsive table-sm ',$this->getCSSclass('navigation', $position),'">',"\n";
	...
	function display_record_buttons($position) 
	{
		if (($but_str = $this->display_buttons($position)) === null)
			return;
		echo "\n".'<!-- lower tabs commented out in function display_record_buttons($position) -->'."\n";
		if ($position == 'down') {
	...
	function display_button($name, $position = 'up') 
	{
		...
		if ($name == 'goto_text') {
			...
			$ret .= ' name="'.$this->cgi['prefix']['sys'].'navpn'.$position.'" id="'.$this->cgi['prefix']['sys'].'navpn'.$position.'" value="'.($current_page+1).'"';
	...
	//if ($fields != false) {
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			...
			if ($this->password($k)) {
				...
				$maxlen > 0 || $maxlen = 500;
				// 2013-03-19 19:45 Bootstrap style replaces size/maxlength requirement
				...
				echo '" type="text" name="'.$this->cgi['prefix']['sys'].'qf'.$k.'" id="'.$this->cgi['prefix']['sys'].'qf'.$k.'"',$len_props;
	...
	function list_table() 
	{
		...
		// table rows listing
		//echo '<table class="',$this->getCSSclass('main'),'">',"\n";
		// Bootstrap tweak<!--  -->
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			...
			if (! $this->fdd[$fd]['sort'] || $this->password($fd)) {
				...
				echo '<a rel="nofollow" class="',$css_class_name,'" href="';
				...
				if ($select_recs) {
					...
					if ($this->nav_graphic_links()) {
						...
						if ($this->view_enabled()) {
							...
							echo 'pme-view.png" height="16" width="16" ';
						...
						if ($this->change_enabled()) {
							...
							echo $css_class_name,'" src="',$this->url['images'];
							echo 'pme-change.png" height="16" width="16" ';
						...
						if ($this->copy_enabled()) {
							...
							echo $css_class_name,'" src="',$this->url['images'];
							echo 'pme-copy.png" height="16" width="16" ';
						...
						if ($this->delete_enabled()) {
							...
							echo $css_class_name,'" src="',$this->url['images'];
							echo 'pme-delete.png" height="16" width="16" ';
	...
	{
		// probably best to use a trigger with PHPMailer
		...
		$body    .= "http://opensource.platon.sk/projects/phpMyEdit/\n\n";
	...
	{
		echo '<h3 class="text-danger">phpMyEdit error: ',htmlspecialchars($message),'</h3>',"\n";
		if ($additional_info != '') {
	...
	function execute() 
	{
		...
		// deprecated under PHP 5.3.1
		// @set_magic_quotes_runtime(0); //removed by Jon
	...
	function phpMyEdit($opts) 
	{
		...
		$this->labels = $this->make_language_labels(isset($opts['language']) ? $opts['language'] : $this->get_server_var('HTTP_ACCEPT_LANGUAGE'));
		// CGI variables
		$this->cgi = @$opts['cgi'];
		$this->cgi['persist'] = '';
...
//		$old_page = ceil($this->fm / abs($this->inc)) + 1;
		...
		if ($this->key_type == 'real') {
			...
			$this->key_delim = '';
		} elseif ($this->key_type == 'int' || $this->key_type == 'tinyint' || $this->key_type == 'smallint' || $this->key_type == 'mediumint') {
